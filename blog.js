...
app.get('/', (req, res) => {
  ...
  res.send('<!DOCTYPE html><html><head><title>Cloud Links Blog</title>' +
    '<style>body{font-family:Georgia,serif;max-width:800px;margin:0 auto;padding:20px}' +
    'h1{color:#1a1a1a;border-bottom:2px solid #0073aa;padding-bottom:10px}a{color:#0073aa}</style></head>' +
    '<body><h1>Cloud Links Blog</h1><p>Cloud authority resources, SEO strategies, and link building insights.</p>' +
    postList + '</body></html>');
});
...
app.get('/post/:slug', (req, res) => {
  ...
  res.send('<!DOCTYPE html><html><head><title>' + post.title + ' - Cloud Links Blog</title>' +
    '<style>body{font-family:Georgia,serif;max-width:800px;margin:0 auto;padding:20px}</style></head>' +
    '<body><p><a href="/">Back to Blog</a></p><h1>' + post.title + '</h1>' +
...
app.listen(PORT, '0.0.0.0', () => console.log('Cloud Links Blog running on port', PORT));
